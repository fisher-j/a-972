...
plot_sum(d_l, years = "init") %>% 
  test_sum() %>% kbl2(caption = "omnibus test, both species")

plot_sum(d_l, years = "init") %>% 
  test_sum() %>%
  filter(KW_p < .05) %>%
  select(-c(aov_p)) %>%
  unnest(c(Tukey, Dunn, Wilcox)) %>% kbl2(caption = "Post-hoc tests, both species")
...
plot_sum(d_l, years = "init", species = "SESE3") %>% 
  test_sum() %>% kbl2(caption = "Omnibus tests, redwood")

plot_sum(d_l, years = "init", species = "SESE3") %>% 
  test_sum() %>%
  filter(KW_p < .05) %>%
  select(-c(aov_p)) %>%
  unnest(c(Tukey, Dunn, Wilcox)) %>% kbl2(caption = "post-hoc tests, redwood")
...
plot_sum(d_l, years = "init", species = "PSMEM") %>% 
  test_sum() %>% kbl2(caption = "Omnibus tests, Douglas-fir")
