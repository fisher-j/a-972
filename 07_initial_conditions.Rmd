...
d_l %>%
  filter(year == "init") %>%
  get_measures() %>%
  make_summary(measures = c("dbh", "ht_p", "density", "ba", "dom_dbh", "p_redwood")) %>%
...
plot_sum <- function(data, species = unique(data$spp),
  ...
  a <- data %>%
    ...
    summarise(
      ...
      p_redwood = sum(ba[spp == "SESE3"]) / sum(ba),
  ...
  left_join(a, b) %>%
  ungroup() %>%
  pivot_longer(c(ba, qmd, density, ht, p_redwood), names_to = "measure")
}
