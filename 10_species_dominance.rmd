...
temp <- dd |> filter(paste(plot, year) %in% sample(paste(plot, year), 1)) 

Ui_tree <- function(focus_dbh, neighbors_dbh) {
  mean(focus_dbh > neighbors_dbh)
}

calc_Ui <- function(x, y, dbh) {
  trees <- data.frame(x = x, y = y)
  idx <- nearest(trees, k = 1)
  sapply( 1:nrow(trees), function(x) Ui_tree(dbh[x], dbh[ idx[[x]] ]) )
}


Ui_plot <- dd |>
  group_by(treatment, year, spp, plot) |>
  summarize(val = mean(calc_Ui(x, y, dbh))) |>
