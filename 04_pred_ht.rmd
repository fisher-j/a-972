...
```{r}
# I'm testing out wheter I should predict height increment based on first or second 
# period dbh measurement. First period seems like a logical choice, but second period
# may capture more information about the tree.
# i've edited these to try to produce two parallel datasets, I hope I can put them back when I am done
# I've removed all trees that have broken or dead tops at any observation
# in order to ensure the same increment values (response values) are used
# in each dataset
...
```{r}
ht_inc_mod1 <- lmer(ht_inc1 ~ log(dbh) * spp + (1 | plot), data = test_d1)
ht_inc_mod2 <- lmer(ht_inc2 ~ log(dbh) * spp + (1 | plot), data = test_d2)
ht_inc_mod12 <- lmer(ht_inc1 ~ log(dbh) * spp + (1 | plot), data = test_d12)

AICc(ht_inc_mod1, ht_inc_mod2)

ht_inc_d <- augment1(test_d, ht_inc_mod)

summary(ht_inc_mod1)
summary(ht_inc_mod2)
summary(ht_inc_mod12)
```
...
```{r}
p <- list()
one <- emmip(ht_inc_mod1, spp ~ dbh, plotit = FALSE, CIs = TRUE, at = list(dbh = seq(10, 60, 2)))
two <- emmip(ht_inc_mod2, spp ~ dbh, plotit = FALSE, CIs = TRUE, at = list(dbh = seq(10, 60, 2)))
three <- emmip(ht_inc_mod12, spp ~ dbh, plotit = FALSE, CIs = TRUE, at = list(dbh = seq(10, 60, 2)))

facet_labs <- c("beginning dbh for 08-13 and 13-18", "ending dbh for 08-13 and 13-18", "beginning dbh for 13-18 only")

pall <- bind_rows(one, two, three, .id = "mod")
pall$mod <- factor(
  pall$mod,
  labels = c(
    "begining dbh for 08-13 and 13-18",
    "ending dbh for 08-13 and 13-18",
    "begining dbh for 13-18 only"
    )
  )
  

pall %>% ggplot(aes(dbh, yvar, color = spp)) +
  geom_line() +
...
d_l %>%
  ...
  filter(
    live,
    get_cond(2, 3, 5) == lag(get_cond(2, 3, 5)),
    year %in% c("13"),
    spp %in% c("SESE3", "PSMEM"),
    is.na(ht)
  )
