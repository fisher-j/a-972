...
# uniqe values for non standard columns
map(veg_species_l, ~ `[`(.x, !names(.x) %in% c("ID", "Plot_ID", "Species"))) %>%
  map(unique)

veg_species <- bind_rows(veg_species_l, .id = "year") %>%
  select(year, plot = Plot_ID, spp = Species, flag = Flag, notes = Corrections) %>%
...
veg_species$spp <- local({
  toFix <- match(veg_species$spp, fix_sp$old)
  fixes <- fix_sp$new[toFix]
  if_else(is.na(fixes), veg_species$spp, fixes)
})


veg_species <- filter(veg_species, !spp %in% c("PSMEM", "SESE3", "ALRU2", "PISI", "TSHE", "UMCA"))

unique(veg_species$spp)
