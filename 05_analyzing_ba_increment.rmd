...
This visualization shows that most pairwise comparisons of species and treatment are significantly different.
...
layout <- c(
  ...
  patchwork::area(119, 219, 200, 299)
)
...
There has been interest in a bar graph that shows the BAI of each species-treatment pair and identifies significant differences between them.
...
emmeans(ba_inc_ref, ~ treatment + spp, type = "response") %>% 
  ...
  ggplot(aes(treatment, response, fill = spp)) +
    geom_errorbar(
      aes(ymin = response, ymax = upper.CL),
      position = dodge,
      width = 0.2
    ) +
    geom_col(position = dodge) +
    scale_fill_manual(values = c("#969696", "black")) + 
    geom_text(aes(label = .group), position = dodge, vjust = -4)
