...
fl1 <- list(
  ...
  log(ba_inc1 + scale_val) ~ log(dbh) * treatment + (1 | tree_id),
  log(ba_inc1 + scale_val) ~ log(dbh) * treatment + cr + (1 | tree_id),
  log(ba_inc1 + scale_val) ~ log(dbh) + treatment + year + (1 | tree_id),
  log(ba_inc1 + scale_val) ~ log(dbh) * treatment + year + (1 | tree_id),
  log(ba_inc1 + scale_val) ~ log(dbh) * treatment + cr + year + (1 | tree_id),
  ...
  log(ba_inc1 + scale_val) ~ log(dbh) + treatment + cr:year + cr:spp + spp + (1 | tree_id)
)
...
Here I'll look at the top model, plot labels refer to diameter increment (cm/year). Unfortuantely the residuals do not apppear to be evenly distributed. I'm not sure that I can use this model.

```{r}
ba_inc_lmm <- lmer(fl1[[16]], data = test_d, REML = TRUE)
...
I'll use dredge from MuMIn to test all possible models with 3 to 10 predictors models
...
```{r}
sjPlot::tab_model(ba_inc_mod, dv.labels = "BA increment")
```

Here I get the marginal estimates and pairwise comparisons for the selected model. If interaction with species and treatment was not included in the model, than we can't compare redwood and douglas-fir response in each treatment.

```{r}

emmeans(ba_inc_mod, pairwise ~ treatment, type = "response")

```

Here I plot the expected response by species and dbh
...
emmip(
  ...
  ggplot(aes(dbh, yvar, color = spp)) +
    ...
    labs(y = "Predicted BA increment", title = "back transformed from log-log model")
