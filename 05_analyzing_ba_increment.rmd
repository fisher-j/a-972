...
```{r}
ba_inc_lmm <- lmer(fl1[[17]], data = test_d, REML = TRUE)
...
Here I plot the expected response by species and dbh, averaged over the effect of year.
...
a <- emmip(
  ...
  mutate(
    treatment = recode_factor(treatment,
      H40 = "H40", L40 = "L40", H80 = "C80", L80 = "L80", C = "Control"
    ),
  ...
  ggplot(aes(dbh, yvar, color = spp)) +
    ...
    labs(
      x = "Dbh (cm)",
      y = expression(Basal ~ area ~ increment ~ (cm^2 ~ year^-1)) 
    # title = "back transformed from log-log model, averaged over year"
    )

```

Here is the expected response resulting from the interaction of year and crown ratio.
...
Can I put these different plots together?

```{r, fig.width=7.16, fig.height=5.91}

layout <- c(
  area(1, 1, 200, 300),
  area(119, 219, 200, 299)
)
