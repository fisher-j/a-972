...
structural_all <- bind_rows(
  ...
  mutate(metric = factor(metric,
    levels = c("SDI*/SDI", "DBHsd", "SH", "SCI", "MDI", "TD")
  ))
```
...
var_table(veg_richness_mean, caption = "Average understory species counts")

var_fig(veg_richness_mean)
...
veg_cover %>% 
  ...
  pivot_longer(-c(year, plot, treatment), names_to = c(".value", "type"), names_sep = "_") %>%
  mutate(
    plot2 = str_extract(plot, "\\d"),
    dom = spp_common_name(dom),
    type = if_else(type == "shrub", "Shrub", "Herb"),
    year = if_else(year == "08", "2008", "2013")
  ) %>%
  relevel_treatment() %>%
  arrange(treatment, plot2, year) %>%
  ggplot(aes(year, dom, group = plot2)) +
    ...
    scale_size_continuous(name = "Cover class (%)", breaks = c(15, 30, 60), limits = c(2.5, 62.5)) +
    scale_x_discrete(expand = expansion(mult = 0.37)) +
    labs(y = "Year")
...
veg_cover %>%
  ...
  ggplot(aes(treatment, fill = dom_shrub, group = fct_reorder(plot, cl_shrub + cl_herb))) +
    stat_count(aes(weight = cl_shrub + cl_herb), position = "dodge") +
    labs(caption = "one bar for each plot indicating dominant species and percent cover")
```

This figure shows the same information as above, but stacked, where the total bar heights indicate the average shrub cover for a treatment and the region heights represent proportion of plots where that species was recorded as dominant, weighted by that plots total cover.
...
It would have been nice to show the same information for herb species, but unfortunately, the fact that sword fern is a significant component and it was recorded inconsistently, makes interpretation of this (and much of the understory data) very difficult.
...
We decided on one combined figure with graphs for total veg cover (shrub and herb combined) and trends in species richness. Richness was calculated above, here is total combined veg cover.

```{r}
# change in total combined percent cover of shrub and herb
cover_veg_plot <- veg_cover %>%
  group_by(treatment, year, plot) %>%
  summarize(val = sum(cl_shrub + cl_herb))

comb_veg_cover_mean <- var_mean(cover_veg_plot)
  
var_table(comb_veg_cover_mean, caption = "Average understory species counts")

var_fig(comb_veg_cover_mean)
```

And the final understory veg figure, this should fit in one column.

```{r, fig.width=3.5, fig.height=7.1}
veg <- bind_rows(
  `Cover (%)` = comb_veg_cover_mean,
