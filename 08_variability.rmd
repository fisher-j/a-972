...
UPDATE: After getting updated understory data, with sword fern separated out, we can show changes in percent shrub cover separately. This graph tells a similar story as the figure above showing dominant species change over time because the circles in that figure indicate total percent shrub cover. This figure synthesizes that trend and makes it more explicit. It displays emmeans of modeled percent cover with `treatment:year` interaction, and random intercept for `plot`.
...
mod <- lmer(val ~ treatment * year + (1 | plot), data = shrub_cover_percent)
summary(mod)
...
shrub_cover_mean |>
  ...
  ggplot(aes(year, emmean, group = treatment)) +
    geom_line(aes(linetype = treatment), position = my_dodge) +
    geom_point(aes(shape = treatment), position = my_dodge) +
    geom_errorbar(
      aes(ymin = emmean - SE, ymax = emmean + SE),
      width = .4,
      position = my_dodge,
