...
```{r}
# my_dodge <- position_dodge(1.2)

# make_sci_fig <- function(data = sci_table) {
...
# make_sh_fig <- function(data = summary_sh(sci_d)) {
#   yr_breaks <- c(2008, 2013, 2018)
#   data %>%
#     # filter(myear != "init") %>%
#     relevel_treatment() %>%
#     mutate(year = fix_year(year)) %>%
#     ggplot(aes(year, sh_n_avg, color = treatment)) +
#       geom_line(size = 1, position = my_dodge) +
#       geom_point(position = my_dodge) +
#     # geom_ribbon(aes(ymin = sh_n_ci_lb, ymax = sh_n_ci_ub, fill = treatment), alpha = 0.2, color = NA) +
#       geom_errorbar(
#         aes(ymin = sh_n_ci_lb, ymax = sh_n_ci_ub),
#         width = .75,
#         position = my_dodge
#       ) +
#       theme_bw() +
