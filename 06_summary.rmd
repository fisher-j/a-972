...
measures$dom_ht <- function(data, species) {
  ...
  data %>%
    ...
    group_by(plot, year) %>%
    slice_max(order_by = ht_p, n = 4, with_ties = FALSE) %>%
    filter(spp %in% species) %>%
    group_by(treatment, year) %>%
    summarise(
...
make_summary <- function(data, measures = c("dbh", "ht_p", "density", "ba")) {
  data %>%
...
fig_include_vars <- c("density", "dbh", "ba", "ht_p")
...
make_fig <- function(data = fig_data) {
  ...
  data %>%
    ...
    ggplot(aes(year, mean, color = spp, shape = spp)) +
      ...
      scale_color_manual("spp", values = c(SESE3 = "black", PSMEM = "#969696")) +
...
non_para_dom_test <- function(metric) {
  ...
  # (plot differences)
  W1 <- spp_dom_cnt(metrics = metric) %>%
    filter(spp == "SESE3", year %in% years) %>%
    group_by(plot) %>%
    mutate(diff = n - lag(n)) %>%
    drop_na() %>%
    ungroup() %>%
