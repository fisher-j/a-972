...
Because there are about 14 observations in 2008 and 2013 that do not have
height measurements, I am omitting these from analysis for now. Also, I am only
looking at trees that have height measurements in 2018. This means that of the
3,538 total height observations, I am only looking at 1,929 over the the three
measurement periods.

For dominant heights, I am using dominant dbh trees *not including broken and
dead tops*. Not all of these have heights. For this reason, there are fewer
than 4 trees per plot.
...
## Percent redwood by BA

```{r}

measures$p_redwood <- function(data, species) {
  if(is.null(species)) species <- unique(data$spp)
  data %>%
    filter(spp %in% species, live | status %in% c(15, 16)) %>%
    group_by(treatment, year, plot) %>%
    mutate(redwood_ba = if_else(spp == "SESE3", ba, 0)) %>%
    summarise(
      p_redwood_plot = 100 * sum(redwood_ba) / sum(ba)
    ) %>%
    summarize(
      mean = mean(p_redwood_plot),
      n = n(),
      std = sd(p_redwood_plot)
    )
}
