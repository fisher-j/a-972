...
measures$dbh <- function(data, species) {
  ...
  data %>%
    ...
    summarise(
      mean = sqrt(mean(dbh^2)),
      n = n(),
      std = sd(dbh, na.rm = TRUE)
    )
}
...
I want to be able to define the order of my measures and treatments in tables and graphs

```{r}

my_sort <- function(data, items = c("treatment", "measure"), fig = FALSE) {
  ...
  data %>% arrange(measure, year, treatment)
}
```
...
make_summary <- function(data, measures = c("dbh", "ht", "density", "ba")) {
  data %>%
...
```{r, fig.height = 6.7, fig.width = 7, echo = FALSE, message=FALSE, warning = FALSE}
...
make_fig <- function(data = fig_data) {
  ...
  data %>%
    ...
    ggplot(aes(year, mean, color = spp, shape = spp)) +
      ...
      scale_y_continuous(breaks = scales::breaks_extended(4, w = c(.2, .2, .3, .3))) +
      scale_color_manual("spp", values = c(SESE3 = "black", PSMEM = "#acacac")) +
      scale_shape_manual("spp", values = c(SESE3 = 16, PSMEM = 17)) +
      scale_linetype_manual("spp", values = c(SESE3 = 2, PSMEM = 1)) +
      labs(y = NULL, x = "Year")
}

make_fig()

ggsave(filename = "figs/summary_fig.pdf", device = cairo_pdf, width = 18.2, height = 15, units = "cm")
```

# Species dominance **WORKING ON THIS FUNCTION**
...
spp_dom_raw <- function(data = d_l, metrics = NULL, years = NULL) {
  ...
  if(is.null(metrics)) metrics <- c("dbh", "ht", "ba_inc2")
  raw <- data %>%
    filter(
...
spp_dom_cnt <- function(...) {
  ...
  ungroup() %>%
  complete(nesting(treatment, plot, year), spp, measure, fill = list(n = 0))
}

spp_dom_pct <- function(...) {
  spp_dom_cnt(...) %>%
    group_by(measure, year, treatment, spp) %>%
    summarise(n = sum(n)) %>%
    mutate(freq = n / sum(n))
}
...
```{r, fig.height=2.75, fig.width=3.48}
...
make_dominance_fig <- function(data = spp_dom_pct(d_l)) {
  data %>%
    my_sort(items = "treatment") %>%
    mutate(measure = factor(measure, levels = c("dbh", "ht", "ba_inc2"))) %>%
    ggplot(aes(year, freq, fill = spp)) +
      ...
      # scale_x_continuous(breaks = yr_breaks) +
      scale_y_continuous(breaks = scales::breaks_extended(4, w = c(.2, .2, .3, .3))) +
      scale_fill_manual("spp", values = c(SESE3 = "black", PSMEM = "#ACACAC")) +
      labs(y = "Proportion of highest ranking trees", x = "Year")
}
