...
calc_sh <- function(x, type = "n") {
    count(spp) %>%
    mutate(p_spp = n / sum(n)) %>%
    summarize(sh_n = -sum(p_spp * log(p_spp)))
}

summary_sh <- function(data, measure = "sh") {
  calc_sh(data) %>%
    summarise(
      across(
        starts_with(measure),
...
# calculate MDI for one tree based on coordinats of neighbors
...
# Given x and y coordinates, find k nearest neighbors of, and calcualte
# MDI for each.
calc_mdi <- function(x, y) {
  trees <- data.frame(x = x, y = y)
  # get matrix of distances
  distances <- dist(trees) %>% as.matrix()
  # get index of 4 nearest for each row (excluding self)
  idx <- apply(distances, 1, function(x) order(x)[2:5])
