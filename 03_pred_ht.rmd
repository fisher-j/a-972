...
plot_groups <- function(
  ...
  print(p +
      ...
      labs(title = "Height increment vs dbh") +
      theme_bw() +
      scale_color_manual(
        values = palette(),
...
plot_groups(str_extract(treatment, "C|H|L"))
```
...
plot_groups(str_extract(treatment, "C|40|80"))
```
...
plot_groups(response = ht, predictor = dbh)
...
```{r, fig.height=7.5, fig.width=10.5}
plot_groups(response = ht, predictor = ht_inc_1)
```
...
plot_groups(response = ht, predictor = ht_13)
