...
p_h %>%
  ...
  select(-c(widest, x, y, treatment, cc, rot, status)) %>%
  group_by(tree_id) %>%
  color_groups()
```
...
```{r, fig.height=8, fig.width=9.5, warning=FALSE}
library(ggpmisc)
d_l %>%
  # add increment data
  group_by(tree_id) %>%
  mutate(
    inc_1 = nth(lead(ht) - ht, 2, order_by = year) / 5,
    inc_2 = nth(lead(ht) - ht, 3, order_by = year) / 5
  ) %>%
  ...
  ungroup() %>%
  ggplot(aes(x = dbh, y = inc_2)) +
    geom_point(size = 1.3, color = "gray30") +
    geom_smooth(method = "lm", color = "gold3", fill = "gray75") +
    stat_poly_eq(
      formula = y ~ x,
